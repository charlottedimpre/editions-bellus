...
def _extract_contenu(parsed: dict) -> str:
    """Extrait le contenu textuel depuis le dict parsé (clé postface ou intro)."""
    contenu = parsed.get("postface") or parsed.get("intro")
    if not contenu:
        raise ValueError("Le contenu de la postface est vide après parsing.")
    return contenu
...
def gen_postface(livre_id: int) -> dict:
    """
    Génère la postface via le LLM avec contrôle du nombre de mots,
    ...
    Returns:
        Le dict parsé de la postface générée.
    """
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        response_content = chat_with_major_error_fallback(
            ...
            context_label="postface" if attempt == 1 else "postface_retry",
        ...
        if not isinstance(parsed, dict):
            raise ValueError("La postface parsée doit être un objet JSON.")
        ...
        if attempt < max_attempts:
            if word_count < MIN_WORDS:
                adjustment_msg = (
                    f"La postface est trop courte ({word_count} mots). "
                    f"Elle doit contenir entre {MIN_WORDS} et {MAX_WORDS} mots. Rallonge-la."
                )
            else:
                adjustment_msg = (
                    f"La postface est trop longue ({word_count} mots). "
    ...
    ajout_bdd_postface(parsed, livre_id)
    print(recup_postface(livre_id))
    return recup_postface_dict(livre_id)
...
def ajout_bdd_postface(parsed: dict, livre_id: int) -> None:
    """Insère ou met à jour la postface en base pour le livre donné."""
...
def recup_postface(livre_id: int) -> str:
    """Retourne la postface sérialisée en JSON, ou un objet vide."""
    postface = Postface.objects.filter(livre_id=livre_id).first()

    if not postface:
        return json.dumps({"postface": []}, ensure_ascii=False, indent=2)

    return json.dumps(
        {
            "postface": [
                {
                    "contenu": postface.contenu,
                    "nb_mots": postface.nb_mots,
                }
            ]
        },
        ensure_ascii=False,
...
def recup_postface_dict(livre_id: int) -> dict:
    """Retourne la postface sous forme de dict Python."""
    return json.loads(recup_postface(livre_id))


def reset_postface(livre_id: int) -> None:
    Postface.objects.filter(livre_id=livre_id).delete()